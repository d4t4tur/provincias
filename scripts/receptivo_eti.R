...
visitantes_dnm <-  read_csv2("/srv/DataDNMYE/turismo_internacional/turismo_internacional_pais.zip",
...
receptivo_origen <-  visitantes_dnm %>%
  filter(turismo_internac == "Receptivo") %>%
  group_by(anio, prov, pais) %>%
  summarise(visitantes = round(sum(casos_ponderados))) %>%
  ungroup() %>%
  mutate(
    prov = case_when(
...
receptivo_origen_nest_data <- receptivo_origen %>% 
  mutate(nombre_prov = prov) %>% 
  nest(nested_column_provs = -prov)
...
tabla_origen <-  function(x) {
    ...
    arrange(-visitantes) %>% 
    mutate(rank = 1:n(),
           pais = ifelse(rank <= 5, pais,"Otros" )) %>% 
    group_by(pais) %>% 
    summarise(visitantes = sum(visitantes)) %>% 
    mutate(orden= ifelse(pais == "Otros",2,1),
           participacion_pais = visitantes/sum(visitantes)) %>% 
    arrange(orden,-visitantes) %>% 
    select(-orden) %>% 
    janitor::adorn_totals() %>% 
    gt()  %>%
    ...
    tab_header(title = md(glue("**Origen de Visitantes {unique(x$nombre_prov)}**")),
               subtitle = md(glue("Visitantes no residentes por pais de origen. Año {unique(x$anio)}"))) %>%
    tab_source_note(source_note = md("**Fuente:** Dirección Nacional de Mercados y Estadísticas a partir de datos de DNM"
                                     #"**Fuente**: DNMyE ({unique(indicadores_tot$fuente)[1]}, {unique(indicadores_tot$fuente)[2]}, {unique(indicadores_tot$fuente)[3]})"
    )
    )
...
map(receptivo_origen_nest_data$nested_column_provs, tabla_origen)
